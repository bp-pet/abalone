...
public interface ServerLobbyProtocol extends ServerProtocol {
     ...
     * for every player:
     * <code> + pm.DELIMITER + pm.PLAYER + pm.DELIMITER + String player1Name + pm.DELIMITER
     *     + String player1TeamName + pm.DELIMITER + String player1Color </code>
     * 
     ...
     * to all clients.
     * If the move is deemed illegal by the server, the server responds only to the
     * client whose turn it is. The client can then try another move: <code>pm.UNEXPECTED_MOVE</code>.
     * 
     * @return protocol message to be send to the user
     */
